...
func (d *Dao) FetchRecords(c context.Context) (list []*model.PrizeRecord, err error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(
			&t.Id, &t.Phone, &t.PrizeId, &t.DrawDate, &t.CreateTime,
		)
		if err != nil {
...
func (d *Dao) SaveRecords(c context.Context, prizeId model.PrizeKey, phone int64, drawDate string) (insertId int64, err error) {
